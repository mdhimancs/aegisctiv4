...
import { NextGenRecommendationsView } from './components/NextGenRecommendationsView';
...
export function App() {
  ...
  const actorsSummary = {
    total: actors.length,
    sponsors: Array.from(new Set(actors.map(a => a.sponsorType))).length,
    sophistication: Array.from(new Set(actors.map(a => a.sophistication))).length,
    motivations: Array.from(new Set(actors.map(a => a.motivation))).length,
  };
  ...
  const handleManualRefresh = () => {
    ...
    if (Math.random() > 0.4) {
      ...
      const newFeedItem: ThreatFeedItem = {
        id: `feed-dynamic-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
  ...
  const handleAddCustomFeedSource = (source: CustomFeedSource) => {
    ...
    // Automatically generate live incoming threat notices for the newly added feed source
    const generatedFeedItems: ThreatFeedItem[] = [
      {
        id: `feed-${Date.now()}-1`,
        provider: source.name as any,
        title: `Ingested ${source.name}: High-confidence C2 beacon signature synced via ${source.format}`,
        indicator: `185.220.${Math.floor(Math.random() * 200) + 10}.${Math.floor(Math.random() * 250) + 1}`,
        indicatorType: 'ip',
        threatType: 'c2',
        severity: 'critical',
        confidence: 94,
        tlp: 'TLP:AMBER',
        timestamp: new Date().toISOString(),
        description: `Automated ${source.format} feed synchronization from ${source.url}. Detected emerging adversary command-and-control channel.`,
        tags: [source.format, 'Auto-Ingested', 'Active-Channel', source.name],
        sourceUrl: source.url,
        rawPayload: JSON.stringify({ source: source.name, format: source.format, status: 'SYNCED', endpoint: source.url })
      },
      {
        id: `feed-${Date.now()}-2`,
        provider: source.name as any,
        title: `Ingested ${source.name}: Exploit weaponization payload hash identified`,
        indicator: Math.random().toString(16).substring(2, 34) + Math.random().toString(16).substring(2, 34),
        ...
        description: `High-entropy binary hash extracted from remote endpoint ${source.name}. Matched against known offensive tooling fingerprints.`,
        tags: [source.format, 'Payload-Hash', source.name],
        sourceUrl: source.url,
        rawPayload: JSON.stringify({ source: source.name, type: 'hash_sha256', verified: true })
      }
    ];

    setFeeds((prev) => [...generatedFeedItems, ...prev]);
    setTelemetry((prev) => ({
      ...prev,
      activeFeedsCount: prev.activeFeedsCount + 1,
      trackedIocsCount: prev.trackedIocsCount + source.indicatorsCount
    }));
  };
