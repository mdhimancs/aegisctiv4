  ...
  return (
    <div 
      className={`fixed inset-0 z-[100] flex justify-end transition-opacity duration-300 ${
        ioc ? 'opacity-100' : 'opacity-0 pointer-events-none'
      }`}
    >
      {/* Backdrop */}
      <div 
      ...
      {/* Drawer Content */}
      <div 
        className={`relative w-full max-w-2xl h-full bg-[#0C1222] border-l border-[#1E2D4A] shadow-2xl flex flex-col overflow-hidden text-slate-100 transition-transform duration-500 ease-out font-mono ${
          ioc ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-[#1A253D] flex items-center justify-between bg-[#090E1A] shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{isEnriching ? 'Enriching via Gemini...' : 'Aegis AI Enrichment'}</span>
            </button>
            <button
              onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-xs">
          ...
          <div className="bg-[#080D18] p-4 rounded-xl border border-[#1C2B47] flex items-center justify-between gap-3">
            ...
            <button
              onClick={handleCopy}
              className="px-3.5 py-1.5 rounded-xl bg-[#141E33] border border-[#1E2D4A] hover:bg-[#1C2A47] text-slate-300 hover:text-white text-xs font-mono font-bold transition-colors shrink-0 cursor-pointer"
              title="Copy value"
            >
              {copied ? <Check className="w-4 h-4 text-emerald-400 inline mr-1" /> : <Copy className="w-4 h-4 inline mr-1" />}
              <span>Copy</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[#0E1628] p-3.5 rounded-xl border border-[#1C2B47] space-y-1">
              ...
              <span className="text-sm font-bold text-cyan-400">{ioc.threatActor || 'Attribution Pending / Under Aegis Investigation'}</span>
            </div>
            <div className="bg-[#0E1628] p-3.5 rounded-xl border border-[#1C2B47] space-y-1">
          ...
          {ioc.recommendedAction && (
            <div>
              <h4 className="font-bold text-red-400 uppercase tracking-wider text-[11px] mb-1.5 flex items-center gap-1.5">
                ...
                <span>Recommended Aegis Containment & SOC Action</span>
              </h4>
