...
export const IOCEnrichmentView: React.FC<IOCEnrichmentViewProps> = ({
  ...
  const filteredAndSortedIOCs = iocs
    ...
    .sort((a, b) => {
      ...
      } else if (sortColumn === 'tlp') {
        const tlpOrder: Record<string, number> = { 'TLP:RED': 4, 'TLP:AMBER': 3, 'TLP:GREEN': 2, 'TLP:WHITE': 1 };
        comparison = (tlpOrder[a.tlp as string] || 0) - (tlpOrder[b.tlp as string] || 0);
      }
  ...
  const handleQuickAddAndEnrich = async () => {
    ...
    try {
      ...
      const createdIOC: IOC = {
        id: `ioc-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
        value: newVal.trim(),
        ...
        asn: enriched.asn,
        tags: enriched.tags || ['Aegis-Enriched'],
  ...
  return (
    <div className="space-y-4 text-slate-900 font-sans">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <h3 className="text-[11px] sm:text-xs font-bold text-slate-900 font-mono">
              IOC Enrichment & Deep Threat Intelligence Repository
            </h3>
            <p className="text-[10px] text-slate-500 mt-0.5 font-mono">
              Real-time IOC lookup, threat score evaluation, and automated Gemini CTI ingestion
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
          ...
          <select
            ...
            className={`border rounded-xl px-3 py-2 focus:outline-none transition-all cursor-pointer font-semibold ${
              selectedType !== 'all' 
      ...
      {isAdding && (
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 shadow-xs space-y-3 font-mono animate-in fade-in duration-150">
                    ...
                    {columns
                      ...
                      .map((col) => {
                        ...
                        if (col.id === 'tlp') {
                          return (
                            <td key={col.id} className={`${densityPadding} font-mono`}>
                              <span className="px-1.5 py-0.5 bg-amber-50 rounded text-[11px] text-amber-800 font-bold border border-amber-200 uppercase whitespace-nowrap">
                                {ioc.tlp}
                              </span>
                            </td>
                          );
                          ...
                          return (
                            <td key={col.id} className={`${densityPadding} font-mono`}>
                              <div className="flex items-center gap-2">
                                ...
                                <div className="w-12 bg-slate-200 h-1.5 rounded-full overflow-hidden border border-slate-200">
                                  <div
