...
export const MitreMatrixView: React.FC<MitreMatrixViewProps> = ({
  ...
  return (
    <div className="space-y-4 text-slate-900 font-sans">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] font-bold text-slate-900 font-mono">
                Aegis Global ATT&CK Enterprise Matrix <span className="text-indigo-600 font-black">v15.1</span>
              </p>
            </div>
          </div>
        </div>
              ...
              {tactic.techniques.length === 0 ? (
                ...
                tactic.techniques.map((tech) => (
                  <div
                    ...
                    className="p-2.5 rounded-xl bg-slate-50 hover:bg-slate-50 border border-slate-200 hover:border-indigo-400 transition-all cursor-pointer group shadow-2xs"
                  >
                    <div className="flex items-center justify-between gap-1 mb-1">
                      <span className="font-mono text-xs font-bold text-indigo-700 group-hover:text-indigo-900">
                        {tech.id}
                      </span>
                      <span
                        className={`text-[10px] font-mono px-1.5 py-0.2 rounded border font-bold uppercase ${getDifficultyColor(
                    ...
                    <h4 className="text-xs font-bold text-slate-900 group-hover:text-red-600 transition-colors font-mono leading-tight">
                      {tech.name}
                    </h4>

                    <p className="text-[10px] text-slate-600 mt-1 line-clamp-2 font-sans">
                      {tech.description}
                    </p>

                    {/* Meta Info Grid */}
                    <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] font-mono border-t border-slate-200 pt-1.5">
                      ...
                      <div className="flex items-center gap-1 min-w-0 max-w-[120px]">
                        <Globe2 className="w-2.5 h-2.5 text-sky-600 shrink-0" />
                        <span className="text-slate-600 truncate group-hover:whitespace-normal group-hover:overflow-visible group-hover:bg-white group-hover:z-10 group-hover:relative break-words">
                          {tech.platforms.join(', ')}
                        </span>
                      </div>
      ...
      {/* Technique Inspection Drawer */}
      <div 
        className={`fixed inset-0 z-[100] flex justify-end transition-opacity duration-300 ${
          selectedTechnique ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
      >
        {/* Backdrop */}
        <div 
        ...
        {/* Drawer Content */}
        <div 
          className={`relative w-full max-w-2xl h-full bg-slate-50 border-l border-slate-200 shadow-2xl flex flex-col overflow-hidden text-slate-900 font-mono transition-transform duration-500 ease-out ${
            selectedTechnique ? 'translate-x-0' : 'translate-x-full'
          }`}
        >
          {/* Header */}
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs font-mono">
              ...
              <div>
                <h4 className="font-bold text-emerald-700 uppercase tracking-wider text-[11px] mb-1.5 flex items-center gap-1.5">
                  ...
                  <span>Aegis Defensive Mitigation & Sensor Visibility</span>
                </h4>
