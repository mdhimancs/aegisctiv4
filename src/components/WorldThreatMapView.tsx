...
import { NextGenRecommendationsView } from './NextGenRecommendationsView';
...
export const WorldThreatMapView: React.FC<WorldThreatMapViewProps> = ({
  ...
  const [activeMapPageTab, setActiveMapPageTab] = useState<'map' | 'next-gen'>('map');
...
Raw Hex Sample: ${selectedNode.hexSample || '45 00 00 e4 2b c0 40 00'}
  ...
  useEffect(() => {
    setPortalTarget(document.getElementById('world-map-filters-portal'));
  }, []);

  const filterControls = (
    <>
      <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 h-7 px-2.5 rounded-lg shadow-inner text-[10px]">
        ...
        <select
          ...
          className={`bg-transparent w-28 truncate text-[10px] font-bold focus:outline-none cursor-pointer px-1.5 py-0.5 rounded-md transition-all ${
            activeCampaignFilter !== 'all' 
              ? 'bg-sky-100 text-sky-800 border border-sky-300 shadow-3xs' 
              : 'text-slate-800 hover:bg-slate-300/30'
          }`}
        >
          <option value="all" className="bg-white text-slate-800">Campaigns</option>
          {campaigns.map((c) => (
            <option key={c.id} value={c.id} className="bg-white text-slate-800">{c.title}</option>
          ))}
        </select>
        <span className="w-px h-3.5 bg-slate-400 shrink-0"></span>
        <select
          value={activeSectorFilter}
          onChange={(e) => setActiveSectorFilter(e.target.value)}
          className={`bg-transparent w-[84px] truncate text-[10px] font-bold focus:outline-none cursor-pointer px-1.5 py-0.5 rounded-md transition-all ${
            activeSectorFilter !== 'all' 
              ? 'bg-sky-100 text-sky-800 border border-sky-300 shadow-3xs' 
              : 'text-slate-800 hover:bg-slate-300/30'
          }`}
        >
          <option value="all" className="bg-white text-slate-800">Sectors</option>
          <option value="Government & Defense" className="bg-white text-slate-800">Gov/Defense</option>
          <option value="Financial Services" className="bg-white text-slate-800">Financial</option>
          <option value="Critical Infrastructure & Energy" className="bg-white text-slate-800">Infrastructure</option>
          <option value="Healthcare & Life Sciences" className="bg-white text-slate-800">Healthcare</option>
        </select>
        <span className="w-px h-3.5 bg-slate-400 shrink-0"></span>
        <select
        ...
        >
          <option value="all" className="bg-white text-slate-800">Severity</option>
        ...
        >
          <option value="all" className="bg-white text-slate-800">Status</option>
          <option value="active_incursion" className="bg-white text-slate-800">Active Incursion</option>
          <option value="contained" className="bg-white text-slate-800">Contained</option>
          <option value="critical_exfiltration" className="bg-white text-slate-800">Critical Exfiltration</option>
          <option value="probing" className="bg-white text-slate-800">Probing</option>
        </select>
      </div>
    </>
  );
  ...
  return (
    <div className="space-y-2 text-slate-900 font-sans" id="world-threat-map-view">
      {portalTarget && createPortal(filterControls, portalTarget)}
      {/* Unified Offensive Intelligence Header */}
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 shadow-xs flex flex-wrap items-center justify-between gap-2.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] leading-none mb-0.5">
              Unified Offensive Intelligence
            </h2>
            <h3 className="text-[11px] sm:text-xs font-bold text-slate-900 font-mono">
              Global Threat Telemetry & Geopolitical Attack Map
            </h3>
            <p className="text-[10px] text-slate-500 mt-0.5 font-mono">
              Real-time cyber warfare globe, cross-border adversary vector tracing, and C2 infrastructure nodes
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 bg-white border border-slate-200 p-2 rounded-xl shadow-xs">
          <div className="px-3 py-1 border-r border-slate-100">
            <div className="flex items-center gap-1 text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
              <Flame className="w-2.5 h-2.5 text-rose-500" /> Incursions
            </div>
            <div className="flex items-end gap-1.5">
              <div className="text-xs font-black font-mono text-slate-900">98.4K</div>
              <div className="text-[9px] text-rose-600 font-black mb-0.5">▲ 1.2K</div>
            </div>
          </div>
          
          <div className="px-3 py-1 border-r border-slate-100">
            <div className="flex items-center gap-1 text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
              <Radio className="w-2.5 h-2.5 text-red-500" /> APT Nodes
            </div>
            <div className="flex items-end gap-1.5">
              <div className="text-xs font-black font-mono text-slate-900">05</div>
              <div className="flex flex-col">
                <div className="text-[9px] text-red-600 font-black leading-none tracking-tighter">CN, RU..</div>
              </div>
            </div>
          </div>
          ...
          <div className="px-3 py-1 border-r border-slate-100">
            <div className="flex items-center gap-1 text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
              <GitBranch className="w-2.5 h-2.5 text-orange-500" /> MITRE
            </div>
            <div className="flex items-end gap-1.5">
              <div className="text-xs font-black font-mono text-slate-900">3 ACTV</div>
            </div>
          </div>

          <div className="px-3 py-1 border-r border-slate-100">
            <div className="flex items-center gap-1 text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
              <Target className="w-2.5 h-2.5 text-emerald-500" /> Targets
            </div>
            <div className="flex items-end gap-1.5">
              <div className="text-xs font-black font-mono text-slate-900">12</div>
            </div>
          </div>
          {onExportSTIX && (
            <button
              onClick={onExportSTIX}
              className="ml-1 px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-600 transition-colors shadow-xs border border-indigo-200 flex items-center justify-center"
      ...
      {containedSuccessMessage && (
        <div className="p-2.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-mono flex items-center justify-between shadow-xs animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
            <span>{containedSuccessMessage}</span>
          </div>
          <button onClick={() => setContainedSuccessMessage(null)} className="px-2.5 py-1 text-emerald-600 hover:text-emerald-900 font-bold">
            Dismiss
          </button>
        </div>
      )}
          ...
          >
            ...
            <AnimatePresence>
              {!isDossierCollapsed && (
                ...
                >
            {selectedNode && (
              <div className="flex border-b border-slate-200 bg-slate-50 text-[10px] font-bold font-mono shrink-0">
                <button
                ...
                >
                  <Brain className="w-3 h-3 text-red-600 animate-pulse" />
                  AI
                </button>
                <button
                ...
                >
                  <Globe2 className="w-3 h-3 text-red-600" />
                  Strategic
                </button>
              </div>
            )}
                  ...
                  {dossierTab === 'stix' && (
                    <div className="flex-1 flex flex-col space-y-4 font-mono text-xs">
                      ...
                      <div className="p-2.5 bg-white border border-slate-200 rounded-lg text-[10px] text-indigo-700 font-mono space-y-1.5 shadow-sm">
                        ...
                        {selectedStixNode === 'actor' && (
                          <pre className="overflow-x-auto select-all leading-normal text-[9px] max-h-[140px] bg-slate-100 p-2.5 rounded-lg border border-slate-200 text-slate-800 scrollbar-thin scrollbar-thumb-slate-300">
{`{
  "type": "threat-actor",
  "spec_version": "2.1",
                        ...
                        <div className="flex gap-1 font-mono">
                          {(['yara', 'sigma', 'snort'] as const).map((fmt) => (
                            <button
                              ...
                              className={`px-3 py-1.5 border-b-2 text-[10px] font-bold font-mono transition-all cursor-pointer ${
...
`alert tcp any any -> ${selectedNode.ip || '198.51.100.42'} ${selectedNode.ports?.[0] || 443} (
    msg: "Aegis CTI: Potential ${(selectedNode.actorName || 'Volt Typhoon')} APT Incursion Inbound";
    flow: established, to_server;
    content: "Operation Shell Control";
    nocase;
    classtype: attempted-admin;
    sid: ${10000000 + Math.abs(getNodeHourIndex(selectedNode.id)) * 133};
    rev: 1;
)`
                          )}
                        </div>
      ...
      <div className="mt-5 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col" id="tightly-packed-threat-table">
        <div className="px-4 py-2 border-b border-slate-200 bg-slate-50 flex justify-between items-center relative overflow-hidden">
          ...
          <span className="text-[9px] font-mono text-slate-500 bg-slate-200/50 border border-slate-200 px-2 py-0.5 rounded font-bold uppercase tracking-wider">{filteredNodes.length} Active Nodes Ingesting</span>
        </div>
        ...
        <div className="mt-1 bg-slate-50 border border-slate-200 rounded-xl p-2.5 shadow-sm relative overflow-hidden">
          <NextGenRecommendationsView
            onNavigateTab={onNavigateTab}
            onExportSTIX={onExportSTIX}
          />
        </div>
      )}
    </div>
