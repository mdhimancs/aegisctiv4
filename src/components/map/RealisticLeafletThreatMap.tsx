      ...
      const customIcon = L.divIcon({
        ...
        html: `
          <div class="relative group cursor-pointer flex items-center justify-center -translate-x-1/2 -translate-y-1/2" id="map-node-${node.id}">
            ...
            <div class="absolute top-6 left-1/2 -translate-x-1/2 pointer-events-none whitespace-nowrap px-1.5 py-0.5 rounded bg-slate-950/90 backdrop-blur-xs border ${isOrigin ? 'border-red-500/50' : 'border-emerald-500/50'} text-[9px] font-mono text-slate-200 shadow-sm flex items-center gap-1 z-20 transition-all duration-200 ${isSelected ? 'opacity-100 scale-100' : 'opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100'}">
              <span class="w-1 h-1 rounded-full ${isOrigin ? 'bg-red-500' : 'bg-emerald-400'} animate-pulse"></span>
  ...
  return (
    ...
    >
      ...
      {hoveredNodeInfo && hoveredPos && (
        <div
          className="absolute z-30 pointer-events-none -translate-x-1/2 -translate-y-full mb-3 px-2 py-1.5 rounded-xl bg-white/95 backdrop-blur-md border border-slate-200 shadow-2xl text-slate-900 text-[11px] font-mono min-w-[190px]"
        ...
        >
          <div className="flex items-center justify-between gap-2 pb-1 border-b border-slate-100">
            <span className={`font-bold flex items-center gap-1 ${hoveredNodeInfo.type === 'threat_origin' ? 'text-rose-600' : 'text-emerald-600'}`}>
              {hoveredNodeInfo.type === 'threat_origin' ? '☠️ APT Threat' : '🎯 Victim'}
            </span>
            <span className={`px-1.5 py-0.2 rounded text-[9.5px] uppercase font-bold border ${
              hoveredNodeInfo.type === 'threat_origin'
                ? 'bg-rose-50 text-rose-700 border-rose-200 shadow-xs'
                : 'bg-emerald-50 text-emerald-700 border-emerald-200 shadow-xs'
            }`}>
              {hoveredNodeInfo.type === 'threat_origin' ? 'APT' : 'VICTIM'}
            </span>
          </div>
          <div className="mt-1.5 space-y-0.5 text-[11px]">
            <div className="text-slate-900 font-sans font-bold text-xs tracking-tight">{hoveredNodeInfo.name}</div>
