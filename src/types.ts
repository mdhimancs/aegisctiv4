...
export interface ThreatActor {
  ...
  mitreD3fend?: Technique[];
  mitreAtlas?: Technique[];
  spiffeSpire?: Technique[];
  description: string;
...
export interface NextGenRecommendation {
  id: string;
  pillar: string;
  title: string;
  shortDesc: string;
  fullRationale: string;
  strategicImpact: 'Critical' | 'High' | 'Strategic Game-Changer';
  timelineQuarter: string;
  status: 'Prototype Ready' | 'In Development' | 'Architected' | 'Under Review';
  roiMetric: string;
  technicalComponents: string[];
  standardsCompliance: string[]; // e.g. STIX 2.1, TAXII 2.1, MITRE ATT&CK v15, NIST CSF 2.0
  keyBenefits: string[];
  interactiveFeatureKey?: string;
}
